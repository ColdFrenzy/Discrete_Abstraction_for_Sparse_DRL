...
def compute_value_function_single(map_name, size, OBST, agents, num_episodes = 10000, gamma: float = 0.8, stochastic: bool = False, save: bool = True, show: bool = True) -> None:
    """
    ...
    Args:
        ...
        agents dict[str, tuple]: A dictionary containing the agents and their initial positions.
    ...
    ag = []
    for agent_name, initial_position in agents.items():
    # Add agents to the environment
        a = AgentRL(agent_name, env)
        a.set_initial_position(initial_position[0], initial_position[1])
        a.add_state_encoder(StateEncoderUAV(a))
        ag.append(a)
    ...
    # Crea la RM
    for a in ag:
        RM_I = RewardMachine(transitions, event_detector)
        a.set_reward_machine(RM_I)
        env.add_agent(a)

    # Avvolgi l'ambiente con il wrapper RMEnvironmentWrapper
    rm_env = RMEnvironmentWrapper(env, ag)
