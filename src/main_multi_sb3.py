...
def main(alg="SAC", map_size=5, gamma = 0.1, seed=42):
    ...
    callbacks: List[BaseCallback] = [custom_callback_1, custom_callback_2, custom_callback_3]
    ...
    if alg == "SACHER":
        ...
        for agent, env in zip(agents, envs):
            models.append(model_class(
                ...
                env,
                learning_starts=1e4,
                tensorboard_log=f"./tensorboards/sac_uav_tensorboard/{map_size}x{map_size}_{seed}/{agent}",
                verbose=2,
                device=device,
            ))
    elif alg == "SAC_DENSE":
    ...
    for agent, model, env, callback in zip(agents, models, envs, callbacks):
        ...
        model.learn(12000, callback=callback)
